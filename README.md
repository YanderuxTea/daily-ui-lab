...
### 41. Git‑Merge‑Conflict‑Resolver Widget

- **Описание:** Интерактивный виджет для разрешения конфликтов Git‑слияния.
  - Отображает текущие и входящие изменения в виде цветных блоков кода.
  - Позволяет выбрать **«Принять текущие»** или **«Принять входящие»** через кнопки рядом с каждой секцией.
  - После выбора отображается только выбранный вариант кода, а остальные части скрываются.
  - Включает заголовок с информацией о ветках, строки‑номера, подсветку синтаксиса и нижний футер с индикатором статуса.
  - Стилизация выполнена на Tailwind CSS с кастомными токенами `gcr-*` (фон, границы, цвета текста, выделения).

- **Тайминг реализации:** ~1.5 часа.

- **Путь:** `/git-merge-conflict-resolver-widget`

<details>
<summary style="font-weight: bold">Превью</summary>

![Preview](/public/git-merge-conflict-resolver-widget.gif)

</details>

### 42. Active‑Security‑Sessions‑Manager Widget

- **Описание:** Виджет управления активными сеансами входа в аккаунт.
  - Отображает список устройств, на которых выполнен вход (устройство, браузер, IP, страна, время последней активности).
  - Текущий сеанс помечен яркой меткой **«Текущий»** (компонент `CurrentSession`).
  - Для остальных сеансов доступна кнопка **«X»** — удаляет выбранный сеанс из списка.
  - При наличии более одного сеанса появляется кнопка **«Завершить все остальные сеансы»**, которая оставляет только текущий.
  - Стилизация выполнена на Tailwind CSS с кастомными токенами `asm-*` (фон, границы, цвета текста, акценты).

- **Тайминг реализации:** ~1 час.

- **Путь:** `/active-security-sessions-manager`

<details>
<summary style="font-weight: bold">Превью</summary>

![Preview](/public/active-security-sessions-manager.gif)

</details>

## 43. Swipe‑to‑Confirm Widget

- **Описание:** Интерактивный компонент подтверждения критических действий через перетаскивание ползунка.
  - Три визуальных варианта: **Primary**, **Danger**, **Warning** – задаются через пропсы `classnameFill`, `classnameSwiper` и `classnameTag`.
  - Пользователь тянет ползунок по треку; при достижении 60 % ширины трека действие считается подтверждённым.
  - После подтверждения ползунок меняет цвет на `bg‑swc‑success`, появляется иконка **Check** и текст **«… выполнено»**.
  - Заполняющая часть трека анимируется в реальном времени, а подсказка постепенно исчезает.
  - Если действие не подтверждено, ползунок возвращается в исходное положение.
  - Стилизация выполнена на Tailwind CSS с кастомными токенами `swc-*` (фон, границы, цвета трека, подсказок).

- **Тайминг реализации:** ~1 час.

- **Путь:** `/swipe-to-confirm`

<details>
<summary style="font-weight: bold">Превью</summary>

![Preview](/public/swipe-to-confirm.gif)

</details>

### 44. Number‑Stepper‑Input Widget

- **Описание:** Интерактивный набор шаговых контроллеров для ввода числовых значений.
  - Каждый контроллер отображает иконку, заголовок и короткое описание.
  - Пользователь может увеличивать/уменьшать значение кнопками «+»/«‑» с шагом `step`.
  - Значения ограничены диапазоном `min … max`; при достижении границы кнопка блокируется.
  - При изменении значения отображается измерение (`measure`), если оно задано (например, `г`, `°C`, `дн`).
  - Анимация изменения числа реализована через **Framer Motion** (`AnimatePresence`, `motion.p`) с плавным скольжением вверх/вниз.
  - Стилизация выполнена на **Tailwind CSS** с кастомными токенами `nsi-*` (фон, границы, цвета текста, подсказок).

- **Данные:**
  - Определены в `data/number-stepper-input/numberStepperData.tsx`.
  - Содержат четыре конфигурации:
    1. **Количество** – количество товаров в заказе (`min: 1`, `max: 25`, `step: 1`).
    2. **Порция** – граммов на приём (`min: 50`, `max: 500`, `step: 50`, `measure: "г"`).
    3. **Температура** – целевая температура (`min: 60`, `max: 100`, `step: 5`, `measure: "°C"`).
    4. **Длительность** – дней подписки (`min: 1`, `max: 30`, `step: 1`, `measure: "дн"`).

- **Компоненты:**
  - `components/number-stepper-input/page.tsx` – страница‑обёртка, рендерит список `Widget`‑ов на основе `stepperData`.
  - `components/number-stepper-input/Widget.tsx` – отдельный контроллер, использует `useState` для текущего значения и анимаций.

- **Тайминг реализации:** ~1 час.

- **Путь:** `/number-stepper-input`
